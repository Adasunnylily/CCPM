...
# 设置要使用的CPU核心数
desired_cpu_count = 1  # 设置为希望使用的CPU核心数

# 限制使用的CPU核心数
os.environ["OMP_NUM_THREADS"] = str(desired_cpu_count)
os.environ["OPENBLAS_NUM_THREADS"] = str(desired_cpu_count)
os.environ["MKL_NUM_THREADS"] = str(desired_cpu_count)
os.environ["VECLIB_MAXIMUM_THREADS"] = str(desired_cpu_count)
os.environ["NUMEXPR_NUM_THREADS"] = str(desired_cpu_count)
...
def asso_opt_main(cfg):
    from models.asso_opt.asso_opt import AssoConceptfcc2c_prob_ma
    from models.select_concept.select_algo import mi_select, clip_score_select, group_mi_select, group_clip_select, submodular_select, random_select,\
        submodular_select_own_23_wcls, submodular_select_own_23_wocls
    ...
    if not os.path.isfile(cfg.cls_name_path):
        ...
        #class2concepts = json.load(open(cfg.concept_root + "gpt3_cub_important.json", "r"))
        save_npy_files(class2concepts, cfg.concept_root)
   
    # concept seletion method
    if cfg.concept_select_fn == "random":
        concept_select_fn = random_select
        print("use random")
      
    elif cfg.concept_select_fn == "RAS_wcls":
        concept_select_fn = submodular_select_own_23_wcls
    elif cfg.concept_select_fn == "RAS_wocls":
        concept_select_fn = submodular_select_own_23_wocls
    ...
    data_module = DataModule(
        ...
        use_own_rmdup=cfg.use_own_rmdup
        )
    ...
    if cfg.test:
        ...
        model = AssoConceptfcc2c_prob_ma.load_from_checkpoint(str(ckpt_path),strict=False)
    ...
    model = AssoConceptfcc2c_prob_ma(cfg, init_weight=th.load(cfg.init_weight_path) if 'init_weight_path' in cfg else None)
    ...
    if not cfg.DEBUG:
        ...
        trainer = pl.Trainer(gpus=1, max_epochs=cfg.max_epochs, \
            callbacks=[checkpoint_callback, ], check_val_every_n_epoch=20, default_root_dir=cfg.work_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model-name',
                        type=str,
                        help='input string argument')
    


    parser.add_argument('--lamba', type=float, default=5e-2,
                        help='dis_loss weight')
    parser.add_argument('--init_kl', type=float, default=0.5,
                        help='initial_kl_weight')
    parser.add_argument('--kl_decay_every', type=float, default=50,
                        help='initial_kl_weight')
    parser.add_argument('--kl_decay_factor', type=float, default=0.9,
                        help='initial_kl_weight')  
    parser.add_argument('--kl_min', type=float, default=0,
                        help='minimal_kl_weight')  
    parser.add_argument('--kl_exp',
                        action='store_true',
                        help='whether to enable debug mode') 
    
    parser.add_argument('--c2c_weight', type=float, default=1e-9,
                        help='c2c_loss_weight')  
    parser.add_argument('--concept2cls_loss',
                        action='store_true',
                        help='whether to enable debug mode') 
    parser.add_argument('--concept2cls_12all',
                        action='store_true',
                        help='whether to enable debug mode') 
    parser.add_argument('--concept2cls_121',
                        action='store_true',
                        help='whether to enable debug mode') 
    parser.add_argument('--concept2cls_122',
                        action='store_true',
                        help='whether to enable debug mode') 
    parser.add_argument('--concept2cls_123',
                        action='store_true',
                        help='whether to enable debug mode') 

    parser.add_argument('--kl_cos',
                        action='store_true',
                        help='whether to enable debug mode') 
    
    parser.add_argument('--kl_cos1',
                        action='store_true',
                        help='whether to enable debug mode') 
    
    parser.add_argument('--adapter_weight', type=float, default=1,
                        help='dis_loss weight')
    
    parser.add_argument('--ma_distance',
                        action='store_true',
                        help='whether to enable debug mode') 
    ...
    cfg.lamba = args.lamba
    
    cfg.init_kl = args.init_kl
    cfg.kl_decay_every = args.kl_decay_every
    cfg.kl_decay_factor = args.kl_decay_factor
    cfg.kl_min = args.kl_min
    cfg.kl_exp = args.kl_exp
    cfg.c2c_weight = args.c2c_weight
    cfg.concept2cls_loss = args.concept2cls_loss
    cfg.concept2cls_121 = args.concept2cls_121
    cfg.concept2cls_122 = args.concept2cls_122
    cfg.concept2cls_123 = args.concept2cls_123
    cfg.concept2cls_12all = args.concept2cls_12all
    cfg.kl_cos = args.kl_cos
    cfg.kl_cos1 = args.kl_cos1

    cfg.adapter_weight = args.adapter_weight

    cfg.ma_distance = args.ma_distance
